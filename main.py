...
def check_winner_status():
    ...
    try:
        ...
        # Inform user:
        pyautogui.alert(title='Error', text=f"Error (check_winner_status): {traceback.format_exc()}")

        # Update system log with error details:
        update_system_log("check_winner_status", traceback.format_exc())
...
def play_game():
    ...
    try:
        ...
        while continue_with_game:
            ...
            # Reset mark lists to beginning-of-game state.  If an error occurs, exit the game:
            if not reset_mark_lists():
                exit()
            ...
            while counter >= 0:
                ...
                if counter % 2 == 0: # User plays.
                    ...
                    while invalid_choice != "BOX MARKED SUCCESSFULLY!":
                        # Update the game board to reflect marks made so far.  If an error occurs,
                        # exit the game:
                        if not update_board(None, ""):
                            exit()
                        ...
                        if box == "" or not (box - 1 >= 0 and box - 1 <= 8):  # User has made an invalid choice.
                            ...
                            if marks[box - 1] == " X " or marks[box - 1] == " O ":  # Selected box is already marked.
                                ...
                                # If an error occurs, exit the game:
                                if not update_box_lists(box, 0):
                                    exit()
                    ...
                    # Update the lists tracking which boxes are marked vs. which boxes are unmarked.
                    # If an error occurs, exit the game:
                    if not update_box_lists(box, 1):
                        exit()

                # Check if either a winner can be declared or a draw has been reached.  If an error
                # occurs, exit the game:
                winner_status = check_winner_status()
                if winner_status == "Error":
                    exit()
                elif winner_status == "":  # Game has not ended.
                    ...
                    # If an error occurs, exit the game:
                    if not update_board(None, winner_status):
                        exit()
...
def reset_mark_lists():
    ...
    try:
        ...
        # Inform user:
        pyautogui.alert(title='Error', text=f"Error (reset_mark_lists): {traceback.format_exc()}")\

        # Update system log with error details:
        update_system_log("reset_mark_lists", traceback.format_exc())
...
def update_board(box, winner):
    ...
    try:
        ...
        # Inform user:
        pyautogui.alert(title='Error', text=f"Error (update_board): {traceback.format_exc()}")

        # Update system log with error details:
        update_system_log("update_board", traceback.format_exc())
...
def update_box_lists(box, player):
    ...
    try:
        ...
        # Inform user:
        pyautogui.alert(title='Error', text=f"Error (update_box_lists): {traceback.format_exc()}")

        # Update system log with error details:
        update_system_log("update_box_lists", traceback.format_exc())
...
def update_system_log(activity, log):
    ...
    try:
        ...
        with open("log_tic_tac_toe_game_" + current_date_time_file + ".txt", "a") as f:
